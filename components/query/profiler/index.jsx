...
export default class QueryProfiler extends React.Component {
  ...
  _computeLatency(queries) {
    queries.forEach(query => {
      if (query.time && query.throughput) {
        query.latency = {
          summary: query.time.summary / query.throughput.summary
        };
      }
    });
  }
  ...
  refreshData() {
    ...
    fetchQueries(
      ...
      const metrics = `host.queries.q|p|e|c|f.${allQueryIds.join(
        '|'
      )}.errors.tput`;
      fetchQueries(
        { from, until },
      ...
      const tputMetrics = queries.map(
        query => `host.queries.${query.action}.${query.id}.tput`
      );
      fetchQueries({ from, until }, allHostsIDs, tputMetrics).then(result => {
        result.forEach(item => {
