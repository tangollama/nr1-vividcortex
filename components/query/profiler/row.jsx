...
export default class ProfilerRow extends React.Component {
  ...
  componentDidMount() {
    fetchQuery(this.state.queryId).then(query =>
      this.setState({ digest: query.digest })
    );
  }
  ...
  render() {
    ...
    const action = query.action
      ? { q: 'Query', e: 'Execute', f: 'Fetch', p: 'Prepare', c: 'Close' }[
          query.action
        ]
      : undefined;
    ...
    return (
      <tr className="vc-row">
        ...
        <td>
          ...
          {query.throughput ? (
            <div>Throughput: {query.throughput.summary.toFixed(4)}</div>
          ) : null}
          {query.time ? (
            <div>Total Time: {query.time.summary.toFixed(4)} </div>
          ) : null}
          {query.latency ? (
            <div>Avg Latency: {query.latency.summary.toFixed(4)} </div>
          ) : null}
        </td>
