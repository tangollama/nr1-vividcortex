...
export const buildQueryURL = ({ from, until, hosts, queryId }) => {
  return `${VIVIDCORTEX_URL}/Default/queries/${queryId}?from=${from}&until=${until}&hosts=${flattenHosts(hosts)}`
}

export const buildProfilerURL = ({ from, until, vcHosts }) => {
  const hostsParam = vcHosts.map(host => `id=${host.id}`).join(' ')

  return `${VIVIDCORTEX_URL}/Default/profiler?from=${from}&until=${until}&hosts=${flattenHosts(vcHosts)}&rank=queries&by=time`
}
