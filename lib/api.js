...
export const fetchHosts = (params, userToken) => {
  ...
  return SaasProxy.VIVIDCORTEX(userToken)
    .get(`/hosts?${toGetParams(params)}&nest=tags`)
...
export const fetchQueries = (params, hosts, metrics, userToken) => {
  ...
  return SaasProxy.VIVIDCORTEX(userToken)
    .post(`metrics/query-series?${toGetParams(params)}`, payload)
    .then(response =>
      response.data && response.data.length
        ? normalizeQueriesResponse(response.data)
        : []
    )
